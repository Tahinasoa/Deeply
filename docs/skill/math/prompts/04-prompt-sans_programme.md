...
- ecrire la sourtie markdown (dans un fichier .md) portant le même nom que le fichier source.
# ENTRÉES FOURNIES

1. manuel : "Nombres et calculs numériques.pdf" (à lire/traiter intégralement)
2. Niveau : seconde
3. Matière : Mathématique
4. Domaine : Nombre et calculs
...
Une compétence ne doit pas inclure les données de l'énoncé (ex: « calculer la valeur de 2x + 3 pour x = 5 ») mais doit être formulée de manière générale (ex: « calculer la valeur d'une expression algébrique pour une valeur donnée d'une variable »).
